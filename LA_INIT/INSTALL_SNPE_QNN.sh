...
adb shell "export ADSP_LIBRARY_PATH=\"${SNPE_ADSP_LIBRARY_PATH}\" && LD_LIBRARY_PATH=\"${SNPE_LD_LIBRARY_PATH}\" ./${TARGET_SNPE_BIN}/snpe-platform-validator --runtime all --testRuntime --debug"
adb shell "export ADSP_LIBRARY_PATH=\"${QNN_ADSP_LIBRARY_PATH}\" && LD_LIBRARY_PATH=\"${QNN_LD_LIBRARY_PATH}\" ./${TARGET_QNN_BIN}/qnn-platform-validator --backend all --testBackend --debug"
